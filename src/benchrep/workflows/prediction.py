...
from benchrep.assembly.schemas import parse_training_config, parse_prediction_config
...
def main() -> None:
    ...
    #
    # # Setup paths
    # model_name = f"{pred_config.model.name}_{pred_config.encoder.name}"
    # if pred_config.decoder is not None:
    #     model_name = f"{model_name}_{pred_config.decoder.name}"
    #
    # run_context = RunContext.create(
    #     output_root=pred_config.run.output_root,
    #     stage=pred_config.stage,
    #     project_name=pred_config.run.project_name,
    ...
    #
    # run_log.info("Run initialized with pred_config from: '%s'", raw_pred_config_path)
    # run_log.info("Run outputs will be saved to: '%s'", run_context.output_dir)
    #
    # # Bookkeeping --- pred_config
    # save_config_records(
    ...
    # L.seed_everything(
    #     pred_config.reproducibility.seed,
    #     workers=pred_config.reproducibility.seed_workers,
    # )
    # run_log.info("Global seed set to %s", pred_config.reproducibility.seed)
    #
    # if pred_config.reproducibility.float32_matmul_precision is not None:
    #     torch.set_float32_matmul_precision(
    #         pred_config.reproducibility.float32_matmul_precision
    #     )
    #
    # # Build model and train
    # datamodule = build_datamodule(
    #     dataset_config=pred_config.dataset,
    #     datamodule_config=pred_config.datamodule,
    #     seed=pred_config.reproducibility.seed)
    #
    # model = build_model(config=pred_config)
    #
    # trainer, checkpoint_callback = build_trainer(
    #     trainer_config=pred_config.trainer,
    #     logger_config=pred_config.logger,
    #     checkpoint_config=pred_config.checkpointing,
    ...
    # if pred_config.inspection.torchview.enabled:
    ...
    #         expand_nested=pred_config.inspection.torchview.expand_nested,
    #         depth=pred_config.inspection.torchview.depth,
    #     )
